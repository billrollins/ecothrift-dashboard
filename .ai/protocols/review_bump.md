...
4. **Initiatives (required — update, not just read).**
   ...
   - **Update the initiative file.** Open the `.ai/initiatives/<name>.md` file for the work being shipped. Mark the relevant phase as complete (add acceptance checkbox, update phase section with what shipped, add test results if available). If the phase has sub-phases (e.g., 4.1A, 4.1B), document each separately.
   - **Update `_index.md`.** Update the active row to reflect current phase completion and what is next. If an initiative is fully complete, do **not** archive it without explicit user confirmation (see Archiving below).
...
5. **Consultant context (required when shipping features).**
   - Update **`.ai/consultant_context.md`** with shipped features: new models, endpoints, commands, UI surfaces, settings, known issues, and gotchas. Keep it dense with pointers to extended docs where appropriate.
   - Update the **phase summary table** to reflect the completed phase.
   - Add any new **gotchas** that consultants or future sessions should internalize.
   - This file is the single-file handoff for external advisors. If a consultant opened it cold after this release, would they have the full picture? If not, fill the gaps.
...
8. **`development.md`** — Setup steps and env vars vs root `.env` and `requirements.txt`.

9. **Domain files** (`backend.md`, `frontend.md`, `inventory-pipeline.md`, `databases.md`, etc.) — Spot-check vs `models.py`, `views.py`, `App.tsx` when you changed those areas.

10. **`retag-operations.md`** / **`inventory-pipeline.md`** — When retag or import commands change.
...
19. Summarize what was done (files, bugs, decisions).

20. Update `.ai/context.md` "Current State" if needed.

21. Update root **`README.md`** steering subsection if initiatives, version story, or onboarding paths changed (see Part A §7).

22. Update relevant `.ai/extended/*.md` when models, routes, or auth changed.

23. If version was bumped, ensure `.version`, `package.json`, and `CHANGELOG.md` are already updated (Part C).

24. Unfinished work: note in `CHANGELOG.md` (`[Unreleased]`) instead of burying only in context. Tie bullets to **initiatives** where possible (`.ai/initiatives/_index.md` for active work; `.ai/initiatives/_archived/ARCHIVE.md` for archived initiatives). If the next session cannot name which initiative continues the work, treat that as **missing steering** — user should clarify or add an initiative.

25. Next session: follow `.ai/protocols/startup.md`.