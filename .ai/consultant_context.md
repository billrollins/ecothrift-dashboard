...
<!-- Last updated: 2026-05-02 (Item Processor pricing audit + `manual-review` Retail column; Final Review mockup pending — `fix_this.md` / directive; `review.0.Bump`) -->
...
|---------|----------------|
| **Recently shipped** | **v2.20.0:** inventory **inbound**—PO list/detail, raw manifest upload, **preprocessing** (standardize → clean → **Final Review**), **receiving** routes and **`for-receiving`** API (**`CHANGELOG [2.20.0]`**). Step 2 **`download-cleanup-csv`** is a **lean pre-AI** export; **`apply-cleanup-csv`** validates and merges into staging **`PreprocessingRow`** **`ai_*`** / **`ai_title`** and optional per-row **`ai_status`** from wide CSV (see **`CHANGELOG [Unreleased]`**, **[`cleanup_csv_contract.md`](reference/cleanup_csv_contract.md)**). Buying valuation / thumbs / list UX: **v2.19.x** and earlier—see **`CHANGELOG`**. |
