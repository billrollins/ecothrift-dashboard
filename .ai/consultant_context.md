...
<!-- Last updated: 2026-05-18 (**v2.24.1** patch: processing gate hotfix; production pushes require semver bump) -->
...
| **Version** | Repo root **`.version`** + **`CHANGELOG.md`**. Current steering version: **v2.24.1**. Production pushes should always carry a semver bump and changelog entry, including patch hotfixes. |
...
|---------|----------------|
| **Recently shipped** | **v2.24.1:** Processing gate hotfix — **`build-processing-data`** / processing-data endpoints return structured validation responses, and Processing is temporarily decoupled from Receiving so staff can run **Orders → upload manifest → Preprocessing → Processing** while Receiving/Disputes stay independent. **v2.24.0:** inbound intake stabilization (migrations **0045–0051**, repair tooling, disputes, timeline/order/receiving safeguards). **v2.23.0:** Item Processor workspace **`search`** persists a lowercased **`ProcessingRow.search_string`** blob (**`0043`**), server filter **`search_string__contains`**, list JSON **`searchString`**, **`rebuild_processing_search_string`** after migrate; manual-review POST mirrors denorm (**`CHANGELOG [2.23.0]`**). **v2.21.1:** Processing data hotfix — **`build-processing-data`** bulk-creates minimal **`ManifestRow`** + **`Item`** rows from **`ProcessingRow`** bookmarks and defers Product/BatchGroup enrichment to avoid Heroku timeouts on large POs (**`CHANGELOG [2.21.1]`**). Buying valuation / thumbs: **v2.19.x**—see **`CHANGELOG`**. |
| **Active initiative** | **[`order_processing_pipeline_rebuild`](initiatives/order_processing_pipeline_rebuild.md)** — umbrella **Orders → Preprocessing → Receiving → Processing → …**; preprocessing **core path shipped**. **Near-term:** Step 3 **Final Review** **mockup-aligned** UI (**[`fix_this.md`](reference/fix_this.md)**, **`final_review_visual_rebuild_directive.md`**); count/tolerance UX per **`extended/ux-spec.md`**. **`Session 15`** (same file) sequences intake rebuild stabilization (**`Current Execution Steps`**) before semver release. |
| **Working tree intake** | **Released in v2.24.0/v2.24.1.** **`order_processing_pipeline_rebuild`** **Session 15** — Django migrations **`0045–0051`**, **`Dispute`** model/API work, manifests-on-PO meta, receiving/processing track timestamps, **`PreprocessingOrder` removed** (**`0047`**), deterministic **`repair_intake_pipeline_pos`** for rollout PO ids, orders API vendor **`Q` filter**. **v2.24.1** explicitly records the post-release processing gate patch. |
...
**Heroku (buying):** Daily `compute_daily_category_stats` (refreshes **`CategoryStats`** including **`need_score_1to99`**, **`recovery_rate`** / **`recovery_*_amount`** / **`good_data_sample_size`** — **v2.17.0** good-data cohort requires **sale, retail, cost** each **0.01–9999** on sold rows; then full valuation for open future-ended auctions unless `--skip-recompute-open`); hourly `scheduled_sweep`. **No** nightly inventory **`recompute_cost_pipeline`** (removed **v2.14.0**) — see **`.ai/extended/development.md`**. **Local dev:** **`scripts/dev/daily_scheduled_tasks.bat`** runs the same buying commands (plus **`watch_auctions`**) against the default DB; not automatic with **`runserver`**. **If `PurchaseOrder.retail_value` is wrong** (e.g. backfill vs **`notes`** JSON **`ext_retail`**), **`Item.cost`** and **v2.17.0** margin aggregates are wrong until **`retail_value`** is fixed and **`recompute_all_item_costs`** + **`compute_daily_category_stats`** run — **`.ai/extended/backend.md`** (Item acquisition cost), **CHANGELOG [2.17.1]**. **v2.18.0:** B-Stock **order-process manifest pulls** and related staff endpoints/commands are **removed**; manifests are **CSV upload only** — **CHANGELOG [2.18.0]**.
...
**Inventory PO manifests:** Staff upload/replace vendor CSV from **`/inventory/orders/:id`** (**Raw Manifest** card → **`POST /api/inventory/orders/{id}/upload-manifest/`**); standardization stays in **Preprocessing**. Offline Step 2 cleanup: **`GET /api/inventory/orders/{id}/download-cleanup-csv/`** returns a **pre-AI** lean CSV (standard row fields + **`base_cost`**/**`ideal_price`**); **`POST …/apply-cleanup-csv`** (or multipart **`upload-cleanup-csv`**) validates and writes cleanup into staging **`ai_*`** / **`ai_title`** and optional **`ai_status`** on **`PreprocessingRow`** (or narrow-schema **`ManifestRow`** when staging is absent; wide Grok-style rows use **relaxed** quality blocking—see contract). Step 3 **Final Review** uses **`preprocessing-review`**; **`finalize-preprocessing`** coalesces **`final_*`** then rebuilds **`ManifestRow`**. **Visual** layout still tracks **[`fix_this.md`](reference/fix_this.md)** (directive); not yet mockup-complete in the shipped SPA. **Item Processor** (**`/inventory/processing/:id`**, **v2.21.0**+) — paginated **`processing-workspace`** + lazy **`processing-row-detail`**; **`v2.23.0`** workspace **`search`** + **`searchString`** (**`ProcessingRow.search_string`**); **`rebuild_processing_search_string`** backfills migrated DBs **after** **`migrate`**; read-only **Manifest pricing audit** accordion (**`GET …/manual-review/`**); **`POST …/manual-review/`** edits mirror **`search_string`** (**`CHANGELOG [2.23.0]`**); **`processing-swap` not shipped**. Optional local adjunct (**gitignored** **`workspace/ai-cleanup-grok/helpers/clean-grok.mjs`**, via **`run.bat`**) emits a single **`.cleaned.csv`** (including **`ai_status`**) via xAI. Pointers: **`CHANGELOG [2.21.0]`**, **`CHANGELOG [Unreleased]`**, **`CHANGELOG [2.20.0]`**, **`.ai/extended/inventory-pipeline.md`**, **[`.ai/reference/cleanup_csv_contract.md`](reference/cleanup_csv_contract.md)**.
...
- **Schema:** **`Auction`**: **`ai_category_estimates`**, **`manifest_category_distribution`**, **`estimated_revenue`**, **`revenue_override`**, **`fees_override`**, **`shipping_override`**, **`estimated_fees`**, **`estimated_shipping`**, **`estimated_total_cost`**, **`profitability_ratio`**, **`need_score`**, **`shrinkage_override`**, **`profit_target_override`**, **`priority`**, **`priority_override`** (no **`thumbs_up`** column — **v2.19.0**; staff votes in **`AuctionThumbsVote`**). List/detail JSON: **`my_thumbs_up`**, **`thumbs_up_count`**. **`CategoryStats`** (**`recovery_rate`**, **`recovery_sold_amount`**, **`recovery_retail_amount`**, **`recovery_cost_amount`**, **`good_data_sample_size`** — live valuation: mix × retail × **`recovery_rate`**; **v2.17.0** cohort). **`PricingRule`** **`sell_through_rate`** (legacy CSV seed only; **not** used by **`recompute_auction_full`**). Staff **`CategoryWantVote`** removed **2026-04**. Seeds: **`seed_pricing_rules`** (CSV + **`AppSetting`** keys), **`seed_marketplace_pricing_defaults`**.
...
- **APIs:** **`GET /api/buying/category-need/`**; **`POST`/`DELETE …/thumbs-up/`** (Admin; response **`my_thumbs_up`**, **`thumbs_up_count`**), **`PATCH …/valuation-inputs/`** (Admin — **v2.19.1** normalizes dollar strings, **400** on invalid); list **`ordering`** (e.g. **`-priority`**, **`-thumbs_up_count`**) and query filter **`thumbs_up`** (current user voted); list/detail serializers **`my_thumbs_up`**, **`thumbs_up_count`**, plus valuation fields (**`valuation_source`**, **`has_revenue_override`**, **`effective_revenue_after_shrink`**, etc.).
...
- **React (v2.9.0+):** **`/buying/auctions`** — valuation columns, filter chips + marketplace multi-select, category-need panel (desktop; **Margin** column + **Profitability** detail — **v2.17.0**), watchlist row tint, stable pagination (**`keepPreviousData`**); **v2.19.0** — thumbs column uses **`my_thumbs_up`** + **`thumbs_up_count`**. **`/buying/auctions/:id`** — valuation card, overrides, AI vs manifest strip; **v2.19.1** — **`useBuyingValuationInputsMutation`** avoids detail cache clobber, manifest **map_fast_cat** debounce no longer hammers detail, inline fields select-all on focus. **`GET /api/buying/watchlist/`** list filters extended (**`profitable`**, **`needed`**, **`thumbs_up`**) to match main list. **Token-backed** B-Stock calls from the **REST API** are **disabled** (`501` / `token_backed_bstock_disabled`) — **CSV upload** + soft-touch sweep; see **`apps/buying/api_views.py`**.
