...
<!-- Last updated: 2026-05-18 (**Session 15** intake wave — working tree; **`CHANGELOG [Unreleased]`**; **`review.0/1/9`** housekeeping; **`order_processing_pipeline_rebuild`** Execution steps; deep-dive **`latest/`**) -->
...
| **Version** | Repo root **`.version`** + **`CHANGELOG.md`** (e.g. **v2.23.0** at time of this edit). |
...
|---------|----------------|
| **Recently shipped** | **v2.23.0:** Item Processor workspace **`search`** persists a lowercased **`ProcessingRow.search_string`** blob (**`0043`**), server filter **`search_string__contains`**, list JSON **`searchString`**, **`rebuild_processing_search_string`** after migrate; manual-review POST mirrors denorm (**`CHANGELOG [2.23.0]`**). **v2.21.1:** Processing data hotfix — **`build-processing-data`** bulk-creates minimal **`ManifestRow`** + **`Item`** rows from **`ProcessingRow`** bookmarks and defers Product/BatchGroup enrichment to avoid Heroku timeouts on large POs (**`CHANGELOG [2.21.1]`**). **v2.21.0:** **`ProcessingRow`** + paginated **`processing-workspace`**, **`processing-row-detail`**, **`workspace_patch`**; **`processing-swap` removed** (**`CHANGELOG [2.21.0]`**). **v2.20.0:** inventory **inbound**—PO list/detail, preprocessing + **receiving** (**`CHANGELOG [2.20.0]`**). Step 2 **`download-cleanup-csv`** / **`apply-cleanup-csv`**: **[`cleanup_csv_contract.md`](reference/cleanup_csv_contract.md)** + **`CHANGELOG [Unreleased]`**. Buying valuation / thumbs: **v2.19.x**—see **`CHANGELOG`**. |
