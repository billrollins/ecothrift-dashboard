<!-- Last updated: 2026-08-11 (finalize_tars_app active; both prior TARS initiatives superseded) -->
...
- **ACTIVE — Finalize TARS App:** [`finalize_tars_app`](initiatives/finalize_tars_app.md) — one 100%-functional TARS MVP in daily use on the floor. Runs **Audit → Design → Code**: [`audit_register.md`](reference/TARS%20Restoration%20Processing%20App/audit_register.md) is reviewed element by element, then a design is agreed, and only then are coding phases written (each a full cycle: code → test → human walkthrough → commit → push → pull prod). **Supersedes both prior TARS initiatives** (`tars_restoration_workspace`, `tars_full_instruction_wizard_guidance` — now closed to new work, reference only).
- **Online Sales MVP (shipped):** archived [`online_sales_mvp`](initiatives/_archived/_completed/online_sales_mvp.md) — public storefront + Graph mail live; Listings / Holds / Customers; verified holds; account portal. **v2.62.0–v2.69.0**.
...
- **Parked — Online Sales Workspace:** [`online_sales_workspace`](initiatives/_archived/_pending/online_sales_workspace.md) — long-term vision beyond MVP (channels/marketing/P&L). Contract: [`.ai/reference/online_sales_workspace/phase_0_contract.md`](reference/online_sales_workspace/phase_0_contract.md).
- **Superseded — TARS decision guardrails:** [`tars_full_instruction_wizard_guidance`](initiatives/_archived/_pending/tars_full_instruction_wizard_guidance.md) — reference only; scope moved to [`finalize_tars_app`](initiatives/finalize_tars_app.md). Contract: [`standalone_studio_contract.md`](reference/TARS%20Restoration%20Processing%20App/standalone_studio_contract.md).
...
- **Superseded — TARS Restoration workspace:** [`.ai/initiatives/_archived/_pending/tars_restoration_workspace.md`](initiatives/_archived/_pending/tars_restoration_workspace.md) — reference only; scope moved to [`finalize_tars_app`](initiatives/finalize_tars_app.md). Studio at `/restoration/tars` (Phases 0–2 + hardening **v2.39.0**); Processing Queue nav removed in favor of Restorations hub.
...
- **Parked — public site launch:** [`.ai/initiatives/_archived/_pending/public_website.md`](initiatives/_archived/_pending/public_website.md) — storefront **Phases 0–4 shipped** (**v2.26.0**); Online Sales MVP go-live (**v2.69.0**) covers deploy + seed. Helcim/pay-online still out of scope (pickup-only). Hostname routing: `ecothrift.us`/`www` → public SPA, `dash.*` → staff.
