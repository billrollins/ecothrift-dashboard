<!-- Last updated: 2026-04-11T14:30:00-05:00 -->
...
- **B-Stock auction intelligence (v2.6.1; Phases 1–4 + 4.1A complete):** Production data and scraping live in **`apps/buying/`** (`workspace/.bstock_token` from **`python manage.py bstock_token`**, or **`BSTOCK_AUTH_TOKEN`** in `.env`; fixed microservice URLs in **`scraper.py`**). **`sweep_auctions`**, **`pull_manifests`**, **`watch_auctions`**, **`renormalize_manifest_rows`**, **`seed_category_mappings`**, **`seed_fast_cat_mappings`**, **`categorize_manifests`**. **Phase 4.1A:** **`ManifestTemplate`**, CSV **`upload_manifest`**, inlined fast-cat seed (343 keys). **Staff React UI:** **`/buying/auctions`** (sortable grid, retail source tooltips, marketplace chip filters), **`/buying/auctions/:id`** (two-column layout, manifest search/filter, full category bar, CSV drop zone), **`/buying/watchlist`**. DRF: list retail annotations, **`manifest_rows`** search/category params, **`category_distribution`** (all categories, no “Other” rollup). **Next: Phase 5** (auction valuation) per [`.ai/initiatives/bstock_auction_intelligence.md`](initiatives/bstock_auction_intelligence.md). Bookmarklet: **`apps/buying/bookmarklet/bstock_elt_bookmarklet.md`**. Notebook workbench: **`workspace/notebooks/bstock-intelligence/README.md`**.
  - **Phase 4.1A UI polish:** Category distribution bar shows all categories (wrapping legend), auction detail two-column layout (metadata + manifest cards), single-click marketplace filtering on list page, manifest search and category filter on detail page.
