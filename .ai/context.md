...
### Working
- All 8 backend apps with models, serializers, views, URLs, admin (including **`apps/buying/`** for B-Stock auction intelligence: Phases **1–5** + **4.1A** + **4.1B** — **`PricingRule`**, **`valuation`** recompute, **`category-need`** / **`category-want`**, AI title-category **`estimate_batch`**, thumbs-up + valuation-inputs PATCH, list ordering/filters; staff DRF **`/api/buying/*`**; watchlist polling + snapshots + price history; **`CategoryMapping`** + manifest categorization + **`category_distribution`**; AI template + AI fast-cat key mapping + usage logging; token **`POST /api/buying/token/`** in DEBUG; React **`/buying/auctions`**, **`/buying/auctions/:id`**, **`/buying/watchlist`** with Phase **5** valuation **UI** (**v2.9.0**); see initiative / **backend.md** / **frontend.md**)
...
- **Historical sell-through — PO extract (v2.7.1):** **`python workspace/notes/to_consultant/extract_po_descriptions.py`** (tracked; **`psycopg2`**, root **`.env`**) pulls all PO rows from V1/V2/V3, parses B-Stock-style descriptions, and writes **`workspace/notes/to_consultant/purchase_orders_all_details.csv`** (consultant copy; same data as **`workspace/data/po_descriptions_all.csv`**) plus aggregations and **`po_description_analysis.md`**. V3 requires **`public.inventory_purchaseorder`** (otherwise skipped with a message). See [historical_sell_through_analysis.md](initiatives/historical_sell_through_analysis.md) and **`CHANGELOG`** **[2.7.1]**.
- **B-Stock auction intelligence (v2.7.0+; Phases 1–5 + 4.1A + 4.1B):** Production data and scraping live in **`apps/buying/`** (`workspace/.bstock_token` from **`python manage.py bstock_token`**, or **`BSTOCK_AUTH_TOKEN`** in `.env`; fixed microservice URLs in **`scraper.py`**; **JWT-backed** REST paths **`pull_manifest`** / **`poll`** etc. **disabled** from the API for ban prevention — **CSV upload** + soft-touch sweep). **`sweep_auctions`**, **`pull_manifests`**, **`watch_auctions`**, **`renormalize_manifest_rows`**, **`seed_category_mappings`**, **`seed_manifest_templates`**, **`seed_fast_cat_mappings`**, **`create_test_auctions`**, **`categorize_manifests`**, **`seed_pricing_rules`**, **`seed_marketplace_pricing_defaults`**, **`estimate_auction_categories`**, **`recompute_buying_valuations`**. **Phase 4.1A:** **`ManifestTemplate`**, CSV **`upload_manifest`**, **`fast_cat_key`** / **`fast_cat_value`**, **`category_confidence`** **`fast_cat`** on lookup; inlined fast-cat seed (**343** keys). **Phase 4.1B:** Claude **`ai_manifest_template`** / **`ai_key_mapping`**; **`POST …/map_fast_cat_batch/`**; **`DELETE …/manifest/`** (rows only); **`workspace/logs/ai_usage.jsonl`** + **`AI_PRICING`**; **`__no_key__`** sentinels excluded from AI batches. **Phase 5:** **`valuation`**, **`ai_title_category_estimate`**, **`category-need`** / **`category-want`**, **`thumbs-up`** / **`valuation-inputs`**, manifest hooks + sweep follow-up; **React** list/detail/category-need valuation **UI** (**v2.9.0**). **Staff React UI:** **`/buying/auctions`**, **`/buying/auctions/:id`** ( **`ManifestUploadProgress`**, four workers, remove manifest, mapping UX), **`/buying/watchlist`**. DRF: list retail annotations, **`manifest_rows`** search/category params, **`category_distribution`**, valuation fields. See [`.ai/initiatives/bstock_auction_intelligence.md`](initiatives/bstock_auction_intelligence.md). Bookmarklet: **`apps/buying/bookmarklet/bstock_elt_bookmarklet.md`**. Notebook workbench: **`workspace/notebooks/bstock-intelligence/README.md`**.
