...
**Current version:** See repo root `.version` (e.g. `v2.6.1`).
...
### Working
- All 8 backend apps with models, serializers, views, URLs, admin (including **`apps/buying/`** for B-Stock auction intelligence: Phases **1–4** shipped; staff DRF **`/api/buying/*`**; watchlist polling + snapshots + price history; **`CategoryMapping`** + manifest categorization + **`category_distribution`**; token **`POST /api/buying/token/`** in DEBUG; React **`/buying/auctions`**, **`/buying/auctions/:id`**, **`/buying/watchlist`** — see **v2.6.0** / initiative)
...
- **B-Stock auction intelligence (v2.6.1; Phases 1–4 + 4.1A complete):** Production data and scraping live in **`apps/buying/`** (`workspace/.bstock_token` from **`python manage.py bstock_token`**, or **`BSTOCK_AUTH_TOKEN`** in `.env`; fixed microservice URLs in **`scraper.py`**). **`sweep_auctions`**, **`pull_manifests`**, **`watch_auctions`**, **`renormalize_manifest_rows`**, **`seed_category_mappings`**, **`seed_manifest_templates`**, **`seed_fast_cat_mappings`**, **`create_test_auctions`**, **`categorize_manifests`**. **Phase 4.1A:** **`ManifestTemplate`**, CSV **`upload_manifest`**, **`fast_cat_key`** / **`fast_cat_value`**, **`category_confidence`** **`fast_cat`** on lookup; inlined fast-cat seed (**343** keys — intentional coverage limits; **Phase 4.1B** expands). **Staff React UI:** **`/buying/auctions`** (sortable grid, retail source tooltips, marketplace chip filters, refetch on return from detail), **`/buying/auctions/:id`** (two-column layout, manifest search/filter, full category bar, CSV drop zone), **`/buying/watchlist`**. DRF: list retail annotations, **`manifest_rows`** search/category params, **`category_distribution`** (all categories, no “Other” rollup). **Manual E2E validation 2026-04** (initiative). **Next: Phase 5** (auction valuation) per [`.ai/initiatives/bstock_auction_intelligence.md`](initiatives/bstock_auction_intelligence.md). Bookmarklet: **`apps/buying/bookmarklet/bstock_elt_bookmarklet.md`**. Notebook workbench: **`workspace/notebooks/bstock-intelligence/README.md`**.
  - **Phase 4.1A UI polish:** Category distribution bar shows all categories (wrapping legend), auction detail two-column layout (metadata + manifest cards), marketplace chip filters on list page, manifest search and category filter on detail page.
