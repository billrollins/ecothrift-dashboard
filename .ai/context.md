<!-- Last updated: 2026-05-18 (**v2.24.1** patch bookkeeping; production pushes require semver bump + CHANGELOG entry) -->
...
**Current release override (2026-05-18):** repo root `.version` is **`v2.24.1`**. **`v2.24.1`** is the same-day PATCH for the post-**`v2.24.0`** production fixes: structured processing-data validation handling and temporary decoupling of Processing from Receiving so staff can run **Orders → upload manifest → Preprocessing → Processing** while Receiving/Disputes remain independent. Going forward, **every production push warrants a semver bump and `CHANGELOG` entry**, even for patch hotfixes.

**Current version:** See repo root `.version` (e.g. **`v2.23.0`**). **`v2.23.0`** — **Item Processor workspace search blob** — persisted lowercased **`ProcessingRow.search_string`** (values-only JSON flatten); **`GET …/processing-workspace`** filter + list **`searchString`**; **`POST …/manual-review/`** mirrors searchable manifest fields onto linked bookmarks; **`manage.py rebuild_processing_search_string`** — **CHANGELOG [2.23.0]**. **`v2.22.1`** — **Item Processor timeout hotfix** — **`processing_row_detail`** on slim **`PurchaseOrderViewSet`** queryset; **`GET /api/inventory/orders/{id}/`** without **`manifest_rows`** prefetch; no hover-triggered **`processing-row-detail`**; **`useProcessingRowDetail`** **retry**/**window-focus refetch** off — **CHANGELOG [2.22.1]**. **`v2.22.0`** — **Item Processor row-first** — order mutations accept **`processing_row_id` / `processing_row_ids`** (dispute **`processing_rows`**, merge, bulk disposition, print-multiple) with server-side resolution to **`ManifestRow`**/**`Item`**; **`processing_data_required`** when bookmarks lack linked lines; merge denorm scoped to touched rows; **`ProcessingDataBuild`** + migration **`0042_processing_data_build`** — **CHANGELOG [2.22.0]**. **`v2.21.1`** — **Processing data hotfix** — **`POST …/build-processing-data/`** now bulk-creates minimal **`ManifestRow`** + **`Item`** rows from **`ProcessingRow`** bookmarks, skips Product matching / Product rollups / BatchGroup creation on the hot path, and removes full-PO duplicate UPC scans from workspace list/patch payloads to avoid Heroku timeouts on large POs — **CHANGELOG [2.21.1]**. **`v2.21.0`** — **Item Processor stability** — **`ProcessingRow`** bookmarks + denormalized queue fields (migrations **`0040`/`0041`**), **`GET …/processing-workspace/`** paginated list (**default 25 rows**), **`GET …/processing-row-detail/`** lazy full row, mutation **`workspace_patch`** merges; **`POST …/processing-swap/`** and **`SwapModal`** **removed** from scope — **CHANGELOG [2.21.0]**, **`extended/inventory-pipeline.md`**, **`extended/frontend.md`**. **`v2.20.0`** — **Receiving + PO inbound** (`GET /api/inventory/orders/for-receiving/` tiered **`expected_delivery`** ordering; **`/inventory/receiving`** → next PO; Raw Manifest CSV + preprocessing + PO dashboard — **CHANGELOG [2.20.0]**, **`order_processing_pipeline_rebuild` Session 5**). **Inventory — in flight (not tag-gated):** three-layer **`PreprocessingRow`**, **`apply-cleanup-csv`** contract, **Final Review** (**`ai_status` shipped**; **mockup layout** per **[`fix_this.md`](reference/fix_this.md)** **pending**) — **`CHANGELOG [Unreleased]`** (**Session 6+** in **[`order_processing_pipeline_rebuild.md`](initiatives/order_processing_pipeline_rebuild.md)**; **Session 15** intake rebuild (migrations **`inventory.0045+`**, **`Dispute`** + **`repair_intake_pipeline_pos`**, orders dashboard vendor **`Q`** filter) documented in **`CHANGELOG [Unreleased]`** + **[`_recon/README.md`](reference/order_processing_pipeline_rebuild/_recon/README.md)**). **v2.19.1** — **Buying valuation overrides** (React Query save race fix, manifest debounce scoping, **`PATCH valuation-inputs`** 400 on bad decimals, **select all** on **Costs & revenue** inline fields) — **CHANGELOG [2.19.1]**, **`.ai/extended/frontend.md`**, **`.ai/extended/backend.md`**. **v2.19.0** — **Buying thumbs per-user** (API **`my_thumbs_up`** + **`thumbs_up_count`**; drop legacy **`Auction.thumbs_up`**, migration **`0020_remove_auction_thumbs_up`**; list **`ordering`** **`thumbs_up_count`**) — **CHANGELOG [2.19.0]**, **`.ai/extended/backend.md`**, **`.ai/extended/bstock.md`**, **`.ai/extended/frontend.md`**. **v2.18.2** — **Buying UX polish** (active auctions list: optimistic cache, archive grace, layout/filters; auction detail: max-bid gauge **0 → break-even**, category mix **Units**, shared tooltips) — **CHANGELOG [2.18.2]**, **`.ai/extended/ux-spec.md`**, **`.ai/extended/frontend.md`**. **v2.18.1** — **Managers see Settings** (canonical roles from Django groups; **`/admin/settings`** on **`ManagerRoute`**) + auth serializer fix — **CHANGELOG [2.18.1]**. **v2.18.0** — **Buying manifests are CSV-only** (order-process pull REST + `pull_manifests*` commands removed; list/detail use upload + delete). **Auction list:** **Top category %**, **P/R %**, category hover (full mix + source), expand-all detail column, compact centered rows — **CHANGELOG [2.18.0]**. **v2.17.1** — **Manifest retail invariant**: **`ManifestRow.retail_value`** is canonically **per-unit MSRP**; aggregates (auction list sort, valuation, manifest mix, detail card) compute extended retail as **`SUM(Coalesce(quantity, 1) × retail_value)`** at query time. CSV/API ingest hardened to divide extended values by qty when only ext is present. New audit/backfill commands **`diagnose_manifest_retail`** and **`normalize_stored_manifest_retail`** — **CHANGELOG [2.17.1]**. **Ops (data):** If **`PurchaseOrder.retail_value`** is understated vs B-Stock listing (check **`notes`** JSON **`ext_retail`** on backfill rows), **`Item.cost`** and category-need **Margin** / **n** are wrong until the PO is fixed and **`recompute_all_item_costs`** + **`compute_daily_category_stats`** run — see **CHANGELOG [2.17.1] Documentation**. **v2.17.0** — **Good-data cohort** for **`CategoryStats`** recovery and averages (sale + retail + cost each **0.01–9999**); category-need table **Margin** column + detail **Profitability** section — **CHANGELOG [2.17.0]**. **v2.16.0** — **CategoryStats recovery rate** (`SUM(sold_for)/SUM(retail_value)` per taxonomy; valuation + category-need UI **Recovery** column) — **CHANGELOG [2.16.0]**. **v2.15.4** — **AI steering + repository hygiene** (protocols, initiatives archive, env template / script cleanup, `commit_message` / push flow) — **CHANGELOG [2.15.4]**. **v2.15.3** — **AI title estimate yield + sweep ergonomics**: removed redundant `title_echo` verify (rows match via `auction_id`), padded cached system block past Haiku 2048-token minimum for `cache_read` pricing, `estimate_auction_categories --missing-both` backfill — **CHANGELOG [2.15.3]**. **v2.15.2** — **Retail-weighted manifest mix**: `manifest_category_distribution` built from retail share per `fast_cat_value` (row-count fallback), Mixed lots & uncategorized redistributed via AI mix when both exist, per-sweep AI estimate cap lifted — **CHANGELOG [2.15.2]**. **v2.15.1** — **Manifest pipeline optimizations**: HTTP session reuse, CategoryStats preload, queryset annotation, lower inter-auction delay, 1-deep prefetch, bulk_create batch_size, dev timelog/benchmark tooling — **CHANGELOG [2.15.1]**. **v2.15.0** — **Auction detail UX v3**: page restructured around decision flow (urgency strip, decision summary, bid reference card, multi-tick gauge, costs input/output split, sell-through color coding, condition chips, compact manifest) — **CHANGELOG [2.15.0]**, **`.ai/extended/ux-spec.md`**. **Historical data backfill** (Phases 0–6) is **complete** and the initiative is **archived** — **[`.ai/initiatives/_archived/_completed/data_backfill_initiative.md`](initiatives/_archived/_completed/data_backfill_initiative.md)**; **Heroku production** has V1/V2→V3 data and **`Item.retail_value`**. **v2.14.0** replaced the legacy nightly **vendor→PO→item** cost commands with **`PurchaseOrder.est_shrink`** and per-line item cost; buying **need** is **`CategoryStats.need_score_1to99`** + auction **`need_score`/`priority`** mix — **CHANGELOG [2.14.0]**, **`.ai/extended/backend.md`**. **v2.14.1** — SOCKS5 proxy hardened for all B-Stock HTTP — **CHANGELOG [2.14.1]**, **`.ai/extended/vpn-socks5.md`**. **Buying UI:** **v2.12.1** polish — [`.ai/initiatives/_archived/_completed/ui_ux_polish.md`](initiatives/_archived/_completed/ui_ux_polish.md); **v2.13.0** sweep; **v2.13.1** desktop grid. **Initiatives:** active **[order_processing_pipeline_rebuild](initiatives/order_processing_pipeline_rebuild.md)** ([`_index.md`](initiatives/_index.md)); prior **`inventory_intake_pipeline`** [**abandoned**](initiatives/_archived/_abandoned/inventory_intake_pipeline.md). **Buying** roadmap and Phase 6 direction in **`.ai/extended/bstock.md`**, **`.ai/initiatives/_archived/_completed/bstock_auction_intelligence.md`** (session history), **`.ai/extended/backend.md`**, **`.ai/extended/frontend.md`**. **`.ai/personas/`** removed (no role prompt files in-repo).
...
│   ├── protocols/          code.0.Startup.md, sql.0.UpdateSchema.md, session.0.Create.md, session.1.Checkpoint.md, code.1.Bearing.md, review.0.Bump.md, review.1.Diff.md, code.9.Push.md, review.9.Deep.md, session.9.Close.md
│   ├── initiatives/        _index.md (active); _archived/ARCHIVE.md + buckets + _protocols/ (lifecycle how-tos)
│   ├── extended/           Deep-dive domain docs (load on demand — keeps agent context small); **`sql/`** `schema_columns_ecothrift.sql` → `schema.csv` + `cli.md`
...
- **Inventory:** POs, M3 processing, preprocessing (standard manifest; **`GET …/download-cleanup-csv`** exports a **lean pre-AI** CSV—standard fields plus **`base_cost`**/**`ideal_price`**—and **`POST …/apply-cleanup-csv`** merges cleanup into **`PreprocessingRow`** **`ai_*`** / **`ai_title`** / optional **`ai_status`** (13-col Grok CSV + JSON row payloads); wide Grok CSV uses **relaxed** upload validation (quality issues in **`soft_warnings`**); **Final Review** (**`PreprocessingReviewTable`**) surfaces **`ai_status`** and clears it on substantive listing/price edits; **`finalize-preprocessing`** coalesces **`final_*`** and rebuilds **`ManifestRow`** (**`ProcessingRow`** queue rows seeded here — **v2.21.0**); **Item Processor** (**`/inventory/processing/:id`**) — paginated **`processing-workspace`** (**`ProcessingRow.search_string`** / list **`searchString`** — **v2.23.0**), lazy **`processing-row-detail`** (**v2.22.1** — slim **`PurchaseOrder`** queryset on server + no **`manifest_rows`** prefetch on **`GET …/orders/{id}/`**; UI loads detail on row click only, **`useProcessingRowDetail`** no retry churn), mutation payloads **row-first** (**`processing_row_id` / `processing_row_ids`** on dispute / merge / bulk disposition / print-multiple — **v2.22.0**), read-only **Manifest pricing audit** (**`GET …/manual-review/`**); **`processing-swap` not shipped**; **visual** stepper/layout still tracks **[`fix_this.md`](reference/fix_this.md)** / **`final_review_visual_rebuild_directive.md`** (**pending**); see **[`cleanup_csv_contract.md`](reference/cleanup_csv_contract.md)** and **`CHANGELOG`**); **v2.12.0** — item list **pagination `count`** cache (`item_list_total_count`); **v2.14.0** — **`Item.cost`** from **`PurchaseOrder.est_shrink`** + listing retail (intake / PO save); backfill **`recompute_all_item_costs`**; legacy cost-pipeline management commands **removed** (see **[2.14.0]**)
...
| Protocols | `.ai/protocols/` — `code.0.Startup.md`, **`sql.0.UpdateSchema.md`**, `session.0.Create.md`, `session.1.Checkpoint.md`, `code.1.Bearing.md`, `review.0.Bump.md`, `code.9.Push.md`, `review.9.Deep.md`, `session.9.Close.md`; consultant handoff — `.ai/extended/consultant_handoff.md`; initiative lifecycle — `.ai/initiatives/_archived/_protocols/README.md` |
