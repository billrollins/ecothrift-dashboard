<!-- Last updated: 2026-09-09 (1.7.0 card surcharge + drawer kick isolation) -->
...
Upload path pattern: `print-server/ecothrift-printserver-setup-v{VERSION}.exe`. **Ship:** [`.ai/protocols/ship-print-server.md`](../protocols/ship-print-server.md) — skip-if-done build, S3, local + Heroku `PrintServerRelease`, and this-PC install. Script: [`printserver/distribute.py`](../../printserver/distribute.py) `--install-local`. Register: `manage.py publish_printserver` (idempotent). Silent install: `python printserver/installer/setup.py --install`. **Current release:** **1.7.0**.
...
- **Cash drawer (1.7.0):** `POST /print/receipt` with `open_drawer` kicks after a successful print. Kick failures are logged and returned as `drawer_opened: false`; the receipt stays `success: true`. Pin is `drawer_pin` in `settings.json` (0 = pin 2, 1 = pin 5), set on `/`. `kick_drawer` reads that setting.
