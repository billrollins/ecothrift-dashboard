...
- **Production print today:** [`format_receipt`](../../printserver/services/receipt_printer.py) ESC/POS bytes → [`printer_manager.send_raw`](../../printserver/services/printer_manager.py) (MUNBYN / POS-80C). Storefront (name, tagline, address, phone, hours, thank-you, Google Review coupon) is **hardcoded** in that module — POS `receipt_data` only supplies the sale (lines, totals, payment, optional `savings`). **`receipt_data.savings`** is `{total, lines:[{label, amount}]}`; when `lines` is non-empty it prints a YOU SAVED block under TOTAL. Legacy `you_saved` (a single number) also prints under TOTAL. Header, policy, and coupon terms are unchanged in 1.6.0. Admin → Settings no longer shows `store_*` / `receipt_header` / `receipt_footer`. GDI `format_receipt_text` / `send_text` is the fallback path only. Policy: final sale + test-before-you-buy. Test receipts use the same layout as live sales.
