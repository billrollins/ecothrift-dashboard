<!-- Last updated: 2026-09-09 (1.8.0 drawer pin both + kick in receipt job) -->
...
| `printLabel`, `printTest`, `printReceipt`, `openCashDrawer`, etc. | Printing. `printReceipt` may return `drawer_opened` (`true`/`false`/`null`). |
...
Upload path pattern: `print-server/ecothrift-printserver-setup-v{VERSION}.exe`. **Ship:** [`.ai/protocols/ship-print-server.md`](../protocols/ship-print-server.md) — skip-if-done build, S3, local + Heroku `PrintServerRelease`, and this-PC install. Script: [`printserver/distribute.py`](../../printserver/distribute.py) `--install-local`. Register: `manage.py publish_printserver` (idempotent). Silent install: `python printserver/installer/setup.py --install`. **Current release:** **1.8.0**.
...
- **Production print today:** [`format_receipt`](../../printserver/services/receipt_printer.py) ESC/POS bytes → [`printer_manager.send_raw`](../../printserver/services/printer_manager.py) (MUNBYN / POS-80C). Storefront (name, tagline, address, phone, hours, thank-you, Google Review coupon) is **hardcoded** in that module — POS `receipt_data` only supplies the sale (lines, totals, payment, optional `savings`, optional CardX fields). **`receipt_data.savings`** is `{total, lines:[{label, amount}]}`; when `lines` is non-empty it prints a YOU SAVED block under TOTAL. Legacy `you_saved` (a single number) also prints under TOTAL. **CardX (1.7.0):** `card_type`, `card_amount`, `card_surcharge`, `card_charged_total`, `card_surcharge_percent` print after payment. Credit shows base + 3% line + charged total + disclosure; debit shows `Card (Debit)` only. Header, policy, and coupon terms are unchanged. Admin → Settings no longer shows `store_*` / `receipt_header` / `receipt_footer`. GDI `format_receipt_text` / `send_text` is the fallback path only. Policy: final sale + test-before-you-buy. Test receipts use the same layout as live sales.
- **Cash drawer (1.8.0):** `POST /print/receipt` with `open_drawer` **prepends** ESC/POS kick bytes to the receipt RAW job (same spooler job). Kick-build failure logs a warning and sets `drawer_opened: false`; the receipt stays `success: true`. `drawer_pin` in `settings.json` is `0` (pin 2), `1` (pin 5), or `both` (default — pulse pin 2 then pin 5). Selectable on `/`, Settings → Printing, and `GET`/`PUT` `/settings`. `POST /drawer/control` accepts optional `pin` for a test kick without saving. Pulse is 60/120 (2 ms units). `kick_drawer` / `build_kick_bytes` read that setting.
...
4. Fixed template → print × 2 and measure designer dimensions.
5. Increment template → print × 3 and verify sequence.
6. AI Create background/structure → approve, save, and print once.
