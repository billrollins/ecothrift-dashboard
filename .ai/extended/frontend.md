...
**Staff routes** (MainLayout): Dashboard; Essentials (**Dashboard**, **Kiosk**); account menu (**Today** `/today`, **Language** EN / ES). On phones (`down('md')`), Dashboard and Today use a slim top bar (hamburger + title) + `PhoneTabBar` (Dashboard / Today) sitting in the layout flow so the page ends at the top of the bar. A routine open on Today (`/today?run=`), and catalog demo / edit, replace that bar with `RoutinePhoneBar`. Staff `/routines` redirects to Today; the catalog is superuser-only (`/routines/catalog`) and add / edit / delete lives in Admin - Routines. **Today** is the punch, the day's routines (runner in place) and **Hours & pay** on every device (`TodayPhone` / `TodayDesk`). `/pay`, `/hr/time-clock` and `/hr/time-history` redirect to `/today?hours=1`. There is no Day at a glance dialog. Desk Dashboard / Today share `FloorPage` (green band + `FloorNav` Dashboard / Today, Settings for Manager+). On phones, `/dashboard` renders `DashboardPhone` (today hero, trend, this week, past weeks, department cards) instead of stacking the desktop widgets. **Buying** (auctions, watchlist, **vendors**, **orders**, preprocessing); Inventory (**Inbound:** receiving, processing); **Retail Floor** (Catalog workbench, quick reprice, floorplans); POS (terminal, drawers, cash, transactions); **Studios** (Manager+: Label Studio, Floorplans, Blog Studio); **Admin** (Manager+: Users at `/admin/users` — Employees first and default for Admin, Customers via `?tab=customers`; Managers only see Customers; then Departments `/admin/departments` (directory + hub; Manager+ edit description/location/manager, Superuser create/rename/deactivate/delete), Shifts, Routines (Routine Control, superuser) `/admin/routines` (`?view=routines|sections`) — see `routines.md`, Time & payroll (superuser) `/admin/time-payroll`, Settings house at `/admin/settings` (System / Printing / Store / Assumptions / Retail QA / Permissions / AI (superuser): `AiPanel.tsx` + `api/aiSettings.api.ts` + `hooks/useAiSettings.ts`)). Floorplan editor AI: `BuildSvgDialog.tsx` (Super Admin, palette sidebar), `AdjustPlanDialog.tsx` (Manager+, AppBar), shared `AiModelEffortFields.tsx`; Apply commits one undo step, the person saves). Super Admin **Enhancements** `/admin/enhancement-requests` is a Restoration guest. Staff **Consignment** routes remain (`/consignment/*`) but are **hidden from sidebar**; **Consignee portal** (`/consignee/*`) unchanged.
...
**Settings + department week grids.** `GET /api/core/settings/` is a full array (`asSettingRows` unwraps a leftover `{ results }` payload). Dashboard department week cells keep a 6px gutter so hover/focus rings are not clipped under the day letters.
...
**Time kiosk (outside MainLayout, `frontend/src/pages/kiosk/`):** **`/kiosk`** (`ProtectedRoute` + `StaffRoute`, lazy `KioskPage`) and **`/clock`** (no guard, lazy `ClockPage`) own the window: dark full-bleed `KioskShell`, board behind, punch overlay in front. Essentials → **Kiosk** opens `/kiosk`. First entry shows **Tap to start** which calls `requestFullscreen()` from the tap and persists `kiosk.started` on `/kiosk` and `clock.started` on `/clock`; the EN / ES toggle persists `kiosk.lang` (both routes read it). Scanner input is a global keydown buffer (`ScanInput`; the rule is the pure `takeScanKey`, tested in `ScanInput.test.ts`). Characters under 400 ms apart build the buffer. Enter or Tab reads it before any gap reset, as long as the last character is under 2 s old: 6 to 32 characters is a scan, a shorter buffer shows "Scan again." and an empty one is silent. No field holds focus, so no on-screen keyboard opens; keys aimed at an input or textarea are left alone. Overlay timeouts: 15 s idle, 45 s while the shift picker is visible (no suggested shift, unmatched, or the picker opened) or while the gate or "Something's wrong" sheet is open. A tap resets either timer. There is no success screen: a punch that lands closes the overlay at once and refreshes the board, and the footer shows a green line (Clocked in, On break, ...) for 4 s. A bad card or a failure shows the same footer line in salmon. A blocked `/clock` shows only the full-screen "Not available here" line. Clock-in gate (`GateSteps`): stale punch → Fix it, missed `gate_on_miss` routines → reason each, unheard nudges → Heard; reasons and acks are held client-side and posted with the clock-in, so walking away saves nothing. `/kiosk` keeps the host JWT alive with a silent refresh. A `401` or `403` from that refresh dispatches `kiosk:host-expired` (never `/login`) and `HostExpiredOverlay` asks for a manager. A `401` with `code: card` is a bad card and is left alone. Exit takes no password: the Exit button or Esc leaves to `/dashboard` (`kioskExit` still writes the audit row). Esc with the punch dialog open cancels the dialog instead. In fullscreen the browser takes the first Esc to leave fullscreen, so the Exit button is the reliable way out. **Sign-in Scan:** `LoginPage` has a **Scan your card** button that lazy-loads `LoginScan` (dark panel, `ScanInput`, `PunchOverlay` on route `clock`, board query off). A punch returns to the form with a green line and nobody signed in. Esc or Cancel returns to the form. All strings live in `i18n/kiosk.ts` with EN + ES (`kiosk.test.ts` fails on a missing Spanish value). Cards are issued from Users → employee drawer → **Kiosk card** (`BadgeBlock`: one-time Code 128 via `jsbarcode`, printed through the local print server).
...
- **Listing photos:** public grid, PDP, cart, and holds use the full gallery file with `object-fit: contain`. Click the PDP photo for `ListingLightbox` (full). Staff can still frame main / grid / thumb in Listing Studio.

**Online Sales (staff):** Listing Studio drop / multi-select opens `ListingPhotoEditorDialog` (Main 1600×1200 default; Full / Grid / Thumb chips; one Apply). `useListingDraft` autosaves title, price, and Facebook fields. Mark posted turns on when Posted URL is entered. Messages (`MessagesPanel`): owner chip + Owner column, Unassign, Mine filter; Resolve asks first. Reply sends `notify` (Email the customer / Post without email). Hold drawer: **Undo Complete** for a staff Complete misclick; **Reopen hold** still needs an internal note.

## Layouts

### MainLayout

- **Sidebar** (**252px**): workspace nav — pinned Essentials (Dashboard) + workspaces (Buying → Processing → Restoration → Retail Floor → Cashier → Deliveries → Online Sales → Studios → Admin); logo, version footer. Waiting-work badges come from `useNavBadgeCounts` (Messages = `needs_reply`; Parts Requests = live approvals / cancel asks / reviews) and roll up onto the workspace pip. **Overflow:** drawer paper and the nav scroll area use **`overflow-x: hidden`** (vertical scroll only); nav list is full-width with **`minWidth: 0`**; long labels **`noWrap`** + ellipsis (see **v2.2.4** `CHANGELOG`).
- **AppBar**: sticky, default color, user avatar + menu (logout)
- **Outlet** for page content
- Mobile: temporary drawer with hamburger toggle. Phone-shell routes (`showsPhoneTabBar`: `/dashboard`, `/today`, the superuser routines catalog) keep the hamburger, show logo + page title, and mount `PhoneTabBar` (Dashboard / Today) as an in-flow footer. A routine open on Today, and demo / edit, hide it so `RoutinePhoneBar` can own the bottom.
- Version in sidebar footer from `getAppVersion()` → `/api/core/system/version/`

**Floor pages (desk):** `FloorPage` + `FloorPageBand` + `FloorNav` (`frontend/src/navigation/floorNav.ts`, `frontend/src/components/layout/FloorPage.tsx`). Band is one 60px row on the runner green gradient: title 20/800, optional inline subtitle or chips, `FloorNav` on the right. The nav is identical on every page (white pill = active); nothing hangs below it. Content column is `maxWidth: 1440` on `dutyColors.paper` except Dashboard, which keeps the olive body. Cards use `dutyCardSx` / `dutyHeroSx` / `ColumnCard`. Phone tabs reuse the same ids and Dashboard / Today labels.

**Today (`/today`):** A compact clock card (clock in / break / clock out), today's routines (`MyWorkList`: Next up, then Do now red / Due soon amber / Later today grey, In progress, Coming up, Anytime, Done today), and Hours & pay. Routines open in place through the URL (`/today?run=12`, or `?routine=&draft=`; `pages/routines/todayRunner.ts`): the desk slides the shift column away and shows the phone runner beside the list, the phone shows it full screen. Old `/routines/run/...` links redirect there. Phone is `TodayPhone`; desk is `TodayDesk`. Counts come from one model (`myWork.ts` / `useMyWork`): the Today badge is everything due today (grey); the app-bar nag (`RoutinesNag`) counts nagging runs plus unheard nudges, takes the colour of the worst, and opens a drawer with Messages (Heard) above the same sections.

**Hours & pay (on Today, `HoursPayPanel`):** This week's hours and hours left, with tags for near or at the weekly limit, flagged shifts and pending time changes. Then the current and past biweekly periods from `GET /hr/time-entries/my_pay/` (empty periods hidden) and recent shifts (tap one for Request time change). One Show pay toggle reveals dollars; it is local `useState` and resets on every visit. The hourly rate is never shown. Open by default on a desk (`?hours=0` folds it), where it fills the column and shows as many shifts as fit; folded by default on a phone (`?hours=1` opens it). **Weekly hours nag** (`hooks/useHoursNag.ts`, `HoursNagCard`): while clocked in, amber with an hour or less left, red at the limit (no overtime is approved); it counts in the nag icon and the Today chip through `components/routines/nagSummary.ts`. Clocking in at the limit asks first (`ClockInLimitDialog`). **Forgotten clock-out:** a punch open 14h+ comes back from `GET /hr/time-entries/current/` with `stale`; the clock card becomes `ForgottenClockOutCard` (pick when you left, default the kiosk suggestion) and `POST /hr/time-entries/{id}/fix_forgotten/` closes it there and files a time change request (`apps/hr/services/forgotten_punch.py`). A plain self clock-out of a stale punch is refused (`stale_punch`).
