...
**Staff routes** (MainLayout): Dashboard; HR (**employees** only in sidebar — time-clock/history/sick-leave routes remain, hidden from nav); Inventory (**Inbound:** orders, preprocessing, receiving, processing, finalization/disputes placeholders; **Items:** search, quick reprice; **Vendors**); POS (terminal, drawers, cash, transactions); **Buying** (`/buying/auctions`, `/buying/auctions/:id`, `/buying/watchlist`); **Admin** (Manager+: assumptions, POS setup, settings; Admin-only: users, customers, permissions). Staff **Consignment** routes remain (`/consignment/*`) but are **hidden from sidebar**; **Consignee portal** (`/consignee/*`) unchanged.
