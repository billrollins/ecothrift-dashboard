<!-- Last updated: 2026-09-18 (settings list + week-cell gutter) -->
...
**Time kiosk (outside MainLayout, `frontend/src/pages/kiosk/`):** **`/kiosk`** (`ProtectedRoute` + `StaffRoute`, lazy `KioskPage`) and **`/clock`** (no guard, lazy `ClockPage`) own the window: dark full-bleed `KioskShell`, board behind, punch overlay in front. Essentials → **Kiosk** opens `/kiosk`. First entry shows **Tap to start** which calls `requestFullscreen()` from the tap and persists `kiosk.fullscreen`; the EN / ES toggle persists `kiosk.lang` (both routes read it). Scanner input is a global keydown buffer (`ScanInput`: fast burst ending in Enter), no focused field. Overlay timeouts: 15 s idle, 45 s with the shift picker or "Something's wrong" sheet open, 4 s on the success screen; any tap resets. Clock-in gate (`GateSteps`): stale punch → Fix it, missed `gate_on_miss` routines → reason each, unheard nudges → Heard; reasons and acks are held client-side and posted with the clock-in, so walking away saves nothing. `/kiosk` keeps the host JWT alive with a silent refresh; on failure `client.ts` dispatches `kiosk:host-expired` (never `/login`) and `HostExpiredOverlay` asks for a manager. Exit needs the host password (`ExitDialog`). All strings live in `i18n/kiosk.ts` with EN + ES (`kiosk.test.ts` fails on a missing Spanish value). Cards are issued from Users → employee drawer → **Kiosk card** (`BadgeBlock`: one-time Code 128 via `jsbarcode`, printed through the local print server).
