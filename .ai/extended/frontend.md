<!-- Last updated: 2026-07-22 (Receiving photo thumbnails, shared Image/CSV viewers, manifest dialog) -->

# Eco-Thrift Dashboard — Frontend Context

**2026-07-21 (v2.50.0) — POS Deliveries release:** Cashier **`/pos/deliveries`** (jobs + available dates + Maps route day, 10-stop warning); **`/pos/printables`** (bilingual policy with fridge/freezer exclusion, Saturday driver log). Online Sales Slot C nav/routes redirected to dashboard (parked). TARS Studio **`/restoration/tars`** remains available; initiative parked.

**2026-07-13 (v2.49.0) — Processing / Restorations hub:** **`/inventory/restorations`** (`RestorationsPage`) — TO setup panel (grade scale/values + handoff via `PATCH` job) and FROM desk (loud Worked/Untouched list + decision panel). Nav label **Restorations** in Processing; legacy `/inventory/restoration-returns` redirects to FROM. Processing check-in with `dispatch=restoration` creates the job and navigates `?lane=to&job=&from=`. Restoration workspace nav drops Queue (TARS Studio Inbox remains at `/restoration/tars`).
...
**2026-07-03 (v2.46.0) — Inventory / Catalog check-in UX:** workbench new check-ins from **Products → Check in items** only; create/duplicate footers match processing (**Check in without printing** / **Check in & print**); edit keeps **Reprint** + **Save** (`ItemCheckInManagePanel`, `InventoryWorkbenchPage`).

**2026-07-03 (v2.46.0) — Floorplan / wall element class:** DB `is_wall` on element kinds (migration **`floorplan.0005`**); wall resize + group scale keep thickness (`palette.ts`, `editorState.ts`, **`ElementKindDialog`**).
...
**Restoration — TARS (`TarsWorkstation`, `/restoration/tars`):** Live bench workstation — item list + parts drawers, evaluation, action log, timer flows. Initiative: [`.ai/initiatives/_archived/_pending/tars_restoration_workspace.md`](../initiatives/_archived/_pending/tars_restoration_workspace.md).
...
- **Shared file viewers:** **`ImageViewerDialog`** and **`CsvViewerDialog`** under `frontend/src/components/common/` (accessible MUI dialogs, loading/error, download actions). Order wrapper **`PurchaseOrderManifestDialog`** lazily calls `getOrderManifestPreview` / `downloadOrderManifest` (blob + **`downloadBlob`**) — reuse from Order detail, Preprocessing (**View raw manifest**), and Processing header when `has_manifest`.
