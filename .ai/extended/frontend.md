<!-- Last updated: 2026-04-29 (**`v2.20.0`** — receiving landing + **`/inventory/receiving`**; `for-receiving`) -->
...
- `PreprocessingPage` (`/inventory/preprocessing/:id`) is a standalone 3-step wizard: Standardize Manifest → AI Cleanup → Manual Review. Has own sidebar nav entry "Preprocessing". localStorage persists last order ID. Legacy route `/inventory/orders/:id/preprocess` redirects.
...
- **AI Cleanup** uses `RowProcessingPanel` with preprocessing-specific model controls (add model id/name, verify, set default), batch size 5/10/25/50, concurrency default **1**, optional 4/8/16 — multi-thread mode is best-effort, and expandable rows showing original data vs AI suggestions.
- **Manual Review** uses `ManualReviewPanel`: searchable linked item/product grid, summary chips for total paid / ideal / set prices / ideal delta, inline item/product edits, and individual/selected/visible ±10% ideal pricing actions.
