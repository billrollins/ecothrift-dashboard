<!-- Last updated: 2026-06-23 (HR Time & payroll, Employees admin) -->
...
**Staff routes** (MainLayout): Dashboard; HR (**Time clock** in Essentials); Inventory (**Inbound:** orders, preprocessing, receiving, processing, finalization/disputes placeholders; **Items:** search, quick reprice; **Vendors**); POS (terminal, drawers, cash, transactions); **Buying** (`/buying/auctions`, `/buying/auctions/:id`, `/buying/watchlist`); **Admin** (Manager+: assumptions, POS setup, settings; Admin-only: **Employees**, customers, permissions; Super Admin: **Time & payroll** `/admin/time-payroll`). Staff **Consignment** routes remain (`/consignment/*`) but are **hidden from sidebar**; **Consignee portal** (`/consignee/*`) unchanged.
