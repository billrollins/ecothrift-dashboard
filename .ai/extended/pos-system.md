...
- **Unified Delivery Day Board**: One board on `/pos/deliveries` Day tab — date rail + persistent cards; **no Board vs Driver / wizard**. **Start delivery day** begins a **run-only** timer (`DeliveryRun`). Stage progresses with guarded CTAs: calls → route → load → active → return → complete (legacy `review`/`truck` remapped; `pos.0017`–`pos.0018`). Same `DeliveryDayCard` shell throughout; stage adds inline actions; card click opens `DeliveryDetailsModal` (full-screen below `lg`, large dialog on desktop — no side drawer). Confirmation = latest call `answered_will_be_there`. Route/ETAs for confirmed stops (DnD); unconfirmed stay visible as excluded. Load/secure + truck photos; drive checkpoints contact → delivered → proof/signature → complete (or report issue → hold); return unload/put-back/reconcile — **does not** mutate inventory `Item.status`. Pre-load reschedule marks stop `rescheduled` (audited); after load, reschedule blocked until issue + return reconcile. Job PATCH `completed` blocked while open-run stop incomplete. APIs under `/pos/delivery-runs/` and `/pos/delivery-stops/` (+ `report-issue`, job `reschedule`, `append-address`, events/`next_action`/`allowed_actions` on run payload).
