...
- **Labor Day / Summer / Assembly**: Header chip shows Labor Day on/off (click to override). Buttons **Assembly · $35** and **Summer · 50%** (checkbox dialog of item/manual lines). Sale chips and sale-savings row on the cart. Receipts print the effective unit price and a ` (10% Labor Day)` / ` (50% Summer)` suffix. `Cart.savings` groups sale and discount dollars; under TOTAL the print server draws a YOU SAVED block from that payload (`receipt_data.savings` + legacy `you_saved`).
