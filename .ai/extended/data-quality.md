<!-- Last updated: 2026-09-24 (bstock Phases 4-6: AUC-01/02 railed by We won it; new fill-ins for units, pallets, close ratio; R-052/R-053 merged) -->
...
| AUC-01 | auction→order | No link between an auction and its PO | 20 of 27 recent POs match an auction title exactly (R-003); no dev PO links by lot id or order number (R-054) | Pipeline by auction, report card, shipping history | **fill:** link by exact title match for analysis; old POs stay unlinked (R-062 checks a backfill) | "We won it" creates the PO with `Auction.purchase_order`, and a "Did we win it?" nag asks about every watched lot that ends (bstock Phase 6) | railed |
| AUC-02 | bids, won | No bids, outcomes or wins recorded; the stored close is the sweep's last price, a median 28 minutes before the end (R-053) | `Outcome` 0, `Bid` 0, watchlist `won` 0 (R-003) | Win rate, price targets, report card, likely close | **use** the last price as the close (a low estimate); wins and losses from "We won it" / "We lost it" from now on | Mark won or lost with the hammer price; the buyer's nags ask for it | railed |
...
| PRD-02 | products | No embeddings or pgvector; the early vector R&D is not in this repo | `pg_extension`: only `pg_trgm` (R-006) | Product matching (bstock Phase 4) | **use** exact and TF-IDF matching for now | Vectors (future) | planned: pgvector in local and Heroku, with fastembed vectors; FAISS optional for batch runs only (owner, 2026-09-24) |
...
| A manifest line that matches no product (or a product with under 3 sales) is valued at the category rate, labelled `value_basis=category`; "% of retail priced from our own sales" is shown | `manifest_analysis.analyze_auction` | PRD-01, ITM-01 |
