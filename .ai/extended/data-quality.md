...
| AUC-05 | auction | `condition_summary` is a list-shaped string, such as `['Used Good']` | all | Condition in triage | **use**, parsed | Store a clean condition value | open |
...
| ITM-11 | items | Product says Mixed lots while the manifest line has a real category | 6,052 items (R-012) | Every per-category number | **fill:** use the manifest's category (free backfill tier) | Prefer the manifest category at check-in | open |
