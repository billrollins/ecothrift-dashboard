<!-- Last updated: 2026-05-06 (inventory_daily_migration + README Related links) -->

# Command line — run SQL (humans + AI agents)

Use this when you want **`ecothrift`** queries **without pgAdmin**. Credentials live in repo-root **`.env`** (see **`.env.example`** — **`DATABASE_*`** keys). Never commit secrets or paste production passwords into chat logs.
...
2. **Credentials:** read **`.env.example`** for variable names; load values from the user’s **`.env`** on disk — do not invent hosts/passwords.
