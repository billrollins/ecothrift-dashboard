<!-- Last updated: 2026-06-09 (quick check-in row path + detailed check-in modal) -->
...
5. **Clean** (Step 2 of PreprocessingPage) — Primary path: **`GET .../download-cleanup-csv/`** exports **`row_id`** as the stable `ManifestRow.id` when available (legacy staging IDs still accepted), plus **`row_number`**, **`quantity`**, **`unit_retail`**, **`base_cost`**, **`ideal_price`**, then standardized **ManifestRow** fields. Upload/apply merges cleaned values into **`PreprocessingRow.ai_*`** / **`ai_title`** (and **`ai_status`** when present) via `manifest_row_id`. **No new `clean_*` layer**; cleaned/human-readable values use existing `ai_*`.
6. **Final Review** (Step 3 of PreprocessingPage; stepper label **Final Review**) — Staging-only review: **`GET/PATCH .../preprocessing-review/`** over **`PreprocessingRow`**, with standardized fallbacks from linked `ManifestRow`. Staff edits **`ai_*`** / **`final_*`** fields and pricing; substantive listing or price **`PATCH`** clears **`ai_status`** server-side. **`POST .../finalize-preprocessing/`** validates price + title/description, then creates/replaces **`ProcessingRow`** bookmarks linked to `manifest_row_id`; it no longer deletes/replaces ManifestRows or creates Items.
...
8. **Processing handoff** — `ProcessingRow` is the fast processor/search surface immediately after finalize. `GET .../processing-workspace/` searches `ProcessingRow.search_string`; row detail can open even when no Items exist yet. The old **Create Processing Data** / chunked build path remains only as a legacy compatibility path for bookmark-only orders.
9. **Arrival check-in** — `POST .../processing-row-check-in/` creates/matches a **`Product`** and creates real **`Item`** rows linked to `purchase_order_id` and `manifest_row_id`, then labels are printed. Over/under quantity is allowed; expected quantity comes from Manifest/Processing rows and actual count comes from Items.
...
- **Check-in**: Primary path is **`POST …/processing-row-check-in/`** from row detail **Quick check-in** or **Detailed check-in** — creates/matches **`Product`** and N **`Item`** rows from manifest/row defaults (no prebuilt intake units required). Repeat check-ins on the same row reuse the latest **`ProcessingCheckInBatch`** product when mode is unset/`keep`. **`POST /api/inventory/items/{id}/processing-print-and-check-in/`** remains for legacy prebuilt **`intake`/`processing`** units only.
- **Print multiple / dispute / merge / bulk disposition** (**v2.22.0 row-first**): order-scoped **`POST`** actions prefer **`processing_row_id` / `processing_row_ids`**; server maps to **`ManifestRow`**/**`Item`**; legacy **`manifest_row_*`** accepted when not conflicting; **`processing_data_required`** (+ HTTP 400 **`code`**) for unlinked bookmarks. UI: **`ProcessingBulkActionBar`** + **`MergeModal`** / **`BulkDispositionModal`** + dispute / print-multiple (**`SwapModal`** not shipped — **v2.21.0**).
