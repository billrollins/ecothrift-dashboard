...
- **Settings:** `workspace/.bstock_token` (from `python manage.py bstock_token`) preferred over `BSTOCK_AUTH_TOKEN`; `BUYING_REQUEST_DELAY_SECONDS`, `BSTOCK_MAX_RETRIES`, `BSTOCK_SEARCH_MAX_PAGES`; **`AI_MODEL`** (fallback only; per-feature models in Settings > AI), **`AI_PRICING`** (see `ecothrift/settings.py` and the **Environment Variables** table in [`development.md`](development.md)). Bookmarklet: `apps/buying/bookmarklet/bstock_elt_bookmarklet.md`
