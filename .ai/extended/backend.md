...
**v2.15.1:** Buying — **Manifest pipeline optimizations**: HTTP session reuse (`_manifest_http_session()`), CategoryStats preload before auction loop, `_has_manifest_rows` annotation (no per-auction `.exists()`), `bulk_create(batch_size=500)`, 1-deep `ThreadPoolExecutor` prefetch, `inter_auction_delay` default 3→1 s. Dev timing: `manifest_dev_timelog.py` + `benchmark_manifest_pull` command. B-Stock manifest page cap: **10 items/page** (ignores `limit` above 10).
...
| **Not in daily batch** | `pull_manifests_nightly`, `pull_manifests_budget` | Manifest queue — separate scripts / windows. Default `--delay` **1.0 s** (v2.15.1, was 3.0). Session reuse + prefetch enabled by default (`MANIFEST_PULL_PREFETCH`; `--no-prefetch` flag). |
