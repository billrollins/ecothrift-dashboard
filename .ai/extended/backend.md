...
- **`POST …/suggest_item/`** and **`POST …/ai_cleanup_rows/`** use the model chosen for `SUGGEST_ITEM` / `INVENTORY_CLEANUP` in **Settings > AI** (was `AI_MODEL_FAST` in `.env`). Per-request - **not** stored in the Django DB cache keys table above.
...
| **KioskEvent** | Audit row for the time kiosk: `at`, `route` (`kiosk` / `clock`), `host` (FK User, hosted only), `subject` (FK User, the card's owner), `action` (`identify_ok` / `identify_fail` / `cooldown` / `clock_in` / `clock_out` / `break_start` / `break_end` / `set_shift` / `request_edit` / `fix_stale` / `gate_cleared` / `unmatched_shift` / `exit`), `shift`, `ip`, `meta` JSON. Never stores the card token. |

**Kiosk API** (`apps/hr/kiosk_service.py` + `kiosk_views.py`). Hosted `/api/hr/kiosk/` (`IsStaff` + `hr.kiosk:use`): `board/`, `identify/` (card token → preview: name, suggested shift from roster, open punch, stale flag, warnings, gate items), `clock-in/` (with `gate: { miss_reasons, nudge_acks }`, committed only on success), `clock-out/`, `break/`, `set-shift/`, `request-edit/` (wrong start time / forgot a break → `TimeEntryModificationRequest`), `fix-stale/` (closes a punch older than 14 h at a suggested time and files a pending request "forgot to clock out"), `exit/`. Public `/api/hr/clock/` (`AllowAny`, device cookie + IP throttle, `kiosk.public_allowed_ips` AppSetting): `board/` (redacted), `identify/`, `clock-in/`, `clock-out/`, `break/`, `fix-stale/`. Boards group by `hr.Department` (`sort_order`, `icon`, inactive skipped); unscheduled punches resolve via `punch_code` → `hr.Shift` → department, fallback "Other". Store-closed warning is suppressed when the person has a shift that day. Missed-routine gate items: `assigned_to` = person, `status=missed`, routine `gate_on_miss` and active, `miss_reason=''`, `due_at` in the past and within 30 days. A successful identify clears the failure counter. `clock_in` and `fix_stale` `select_for_update` the user row before reading the open punch. A gated run that is gone, or not assigned to that person, raises `KioskError` 400.
...
| **RestorationPartsOrderLine** | Bought line: qty, unit_cost, `inspect_verdict` (`acceptable` / `issues` / blank), `inspect_note`. Migrations `0095` / `0096`. |
...
| **ItemNote** | Append-only item-scoped ledger (`note_trail`). `body`, `surface` (`check_in` / `handoff` / `queue` / `action` / `hold` / `send_back` / `reject` / `finish` / `output` / `processing_return` / `manual`), `source_key` (supersede chain), soft `restoration_job_id` (integer — split/combine delete jobs), `check_in` SET_NULL, author, `occurred_at`, status `active`/`revised`/`voided`, `supersedes` PROTECT. Service: `apps/inventory/services/item_notes.py`. **Append** writes a row; empty body is a no-op. **Revise** (manual, author's own) marks the old row `revised` and appends a successor. **Void** (manual, author's own) voids in place and keeps the body. System surfaces are written only by the form that owns them (`record_surface_note_for_job`). Scalar fields stay authoritative for the current value. `Item.notes` is the printed-tag note, not this ledger. |
...
inventory.ItemHistory, inventory.ItemNote, inventory.ItemScanHistory → inventory.Item
...
- **Resolution** (`apps/core/ai_config.py`): `ai_model(purpose, override)` = override, then the active assigned model, then the fallback (`LABEL_IMAGE`: `grok-imagine-image-quality`; everything else: `AI_MODEL`). Per-purpose `AI_MODEL_*` env vars are gone (copied into Settings by `core/0006`). `ai_effort(purpose)` returns the saved effort. Every DB read falls back to env on any exception (SimpleTestCase, migrate, import time).
...
- **Manifest pull (bstock_daily_buying, 2026-09-22):** **`BStockToken`** (the owner's handed-over B-Stock JWT, newest row only, never serialized), **`ManifestPullJob`** (status queued / running / done / failed / stopped, **`runner`** ownership token, **`heartbeat_at`**, **`auction_ids`** fixed at first run, **`results`**), **`ManifestPullLog`** (one row per attempt). **`Auction.manifest_source`** (`auto` / `manual` / `''`), **`manifest_pull_attempted_at`**, **`manifest_pull_error`**, **`manifest_pull_blocked`** (never retried); the new non-null columns carry **`db_default`** because **`sweep_upsert`** inserts with raw SQL. Services **`bstock_token_store`** (validate, locked swap, usable-time status), **`manifest_pull`** (shortlist, per-auction row-locked claim, save, mapping + valuation, jobs, stop, prune). Command **`pull_shortlist_manifests`** (Scheduler every 10 min: resume dead jobs, prune; never starts a pull). API (superuser): **`/api/buying/bstock-login/`** GET status, POST `{token}`, DELETE disconnect; **`/api/buying/manifest-pulls/`** GET `?job=`, POST start/resume, DELETE stop. Detail serializer adds **`manifest_pull_eligible`**. Full flow: [`bstock.md`](bstock.md) → Manifests.
- **Settings:** token order for authenticated calls is the **`BStockToken`** row, then `workspace/.bstock_token` (from `python manage.py bstock_token`), then `BSTOCK_AUTH_TOKEN`; AppSettings **`buying_manifest_pull_window_hours`** / **`_max_per_run`** / **`_retry_hours`** / **`_page_delay_ms`** (Admin → Assumptions); `BUYING_REQUEST_DELAY_SECONDS`, `BSTOCK_MAX_RETRIES`, `BSTOCK_SEARCH_MAX_PAGES`; **`AI_MODEL`** (fallback only; per-feature models in Settings > AI), **`AI_PRICING`** (see `ecothrift/settings.py` and the **Environment Variables** table in [`development.md`](development.md)). Bookmarklet: `apps/buying/bookmarklet/bstock_elt_bookmarklet.md`
