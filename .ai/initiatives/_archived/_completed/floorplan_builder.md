...
- **Wall cutter (v2.47.0, 2026-07-03):** `cut` tool (`Tool` union, toolbar scissors, key **C**) — hover shows snapped dashed cut line, click runs `cutWallAt` (pure, rotation-aware split along length; min-piece guard; copies kind/thickness/rotation/flip/group; new id for second piece); only wall-kind elements respond, locked walls ignored. 3 new vitest cases (229 OK). Also fixed UTF-8 BOM in root `package.json`.
- **Wall element class + selection sizing (v2.46.0, 2026-07-03):** `FloorPlanElementKind.is_wall` (migration `0005`, seeded wall flagged, kind-dialog toggle) — thickness = raw h survives rotation, drag-resize changes length only, group scale keys off the flag (aspect heuristic only for unknown kinds); typed **selection size** W×H with aspect-lock (`resizeSelection` → `scaleObjects`); composition-aware bulk panel (walls/elements/other counts, bulk **Thickness**, image tools scoped to non-walls); single-wall Length/Thickness panel; **`WALL_COMPOSITES`** (L/U/room of grouped 8' segments) + `compositeToElements`. 6 new tests (backend 14 OK, vitest 226 OK). Note: parallel Cursor session committed this batch together with its Catalog check-in work (`f737d17`); release cut on top.
