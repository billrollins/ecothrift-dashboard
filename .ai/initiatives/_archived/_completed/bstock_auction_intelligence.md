...
- **UI (v2.6.1):** Auction list — sortable grid, retail source tooltips, marketplace chip filters, **refetchOnMount** when returning from detail. Auction detail — two-column layout, **Category Mix** bar + **wrapping** legend; **Manifest Rows** with search + fast-category filter; CSV drop zone. **Steering:** `.ai/consultant_context.md`; [`.ai/extended/frontend.md`](../../../extended/frontend.md) / [`backend.md`](../../../extended/backend.md).
