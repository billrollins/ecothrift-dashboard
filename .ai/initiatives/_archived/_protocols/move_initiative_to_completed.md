...
6. **[`.ai/protocols/session.9.Close.md`](../../../protocols/session.9.Close.md)** — align **`.ai/context.md`**, **`CHANGELOG.md`** (document shipped work in `[Unreleased]` or a release section when appropriate), and extended docs; **product semver** bumps follow **shipped behavior**, not “one bump per initiative file” — see [`.ai/initiatives/_index.md`](../../_index.md).
