 ...
 * Fixed costs are overrides, a B-Stock shipping quote, and labor and disposal (Assumptions).
 * Without the rates (older payloads) fees and shipping are taken as fixed at the current price.
 */
export function computeMaxBidAtProfitFactor(detail: BuyingAuctionDetail, factor: number): number | null {
  ...
  const handling = rateOrNull(detail.handling_cost) ?? 0;
  const fixed = (feeRate == null ? fees : 0) + (shipRate == null ? ship : 0) + handling;
