 ...
 * The staff directory.
 *
 * Who they are, what they do, how long they have been here, and whether they
 * can actually sign in. The access cell is always painted and only changes
 * colour, so a row never resizes when someone is deactivated.
 */
...
import { useCreateUser, useUsers } from '../../../hooks/useEmployees';
...
function jobLine(user: User): string {
  ...
  const bits = [profile.position, EMPLOYMENT_LABELS[profile.employment_type]].filter(Boolean);
...
export default function EmployeesPanel({ onSelect }: Props) {
  ...
  const rows = useMemo(
    () => (data?.results || []).filter((u) => STAFF_ROLES.includes(u.role as UserRole)),
    [data],
  );
  ...
  const columns: GridColDef<User>[] = [
    ...
    },
    {
      ...
      renderCell: ({ row }) => (
        <StackCell
          top={row.employee?.position || '—'}
          bottom={
            row.employee
              ? row.employee.department_name ||
                EMPLOYMENT_LABELS[row.employee.employment_type] ||
                'No department'
              : 'No employee record'
          }
        />
