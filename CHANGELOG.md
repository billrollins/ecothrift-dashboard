...
<!-- Last reviewed: 2026-05-02 (`review.0.Bump` — Item Processor read-only pricing audit + `manual-review` Retail column; no semver) -->
# Changelog
...
- **Inventory / Item Processor (workspace)** — Backend: **`GET /api/inventory/orders/{id}/processing-workspace/`** (manifest rows + nested items/products + progress); **`POST …/processing-print-multiple/`**, **`…/processing-dispute/`**, **`…/processing-merge-rows/`**, **`…/processing-swap/`**, **`…/processing-bulk-disposition/`**; **`POST /api/inventory/items/{id}/processing-print-and-check-in/`** (optional **`applyRetailAll`** / **`applyConditionAll`**); **`PATCH …/processing-patch/`** (edit on-shelf unit); **`Item`** dispute fields + **`ProductMergeAudit`** / **`ItemSwapAudit`** (migration **`0039_item_processor_disputes_and_audits`**). Frontend: **`ProcessingWorkspacePage`** at **`/inventory/processing/:id`** (queue, filters, active card, print-after-check-in via **`localPrintService`**, print-multiple + dispute modals, session log placeholder, close PO); **`ProcessingEntryRedirect`** at **`/inventory/processing`** ( **`?order=`** legacy query or last-used PO); legacy grid at **`/inventory/processing-legacy`**. Tests: **`apps/inventory/tests/test_processing_validation_matrix.py`** covers **V-02**, **V-26–V-31**, **V-35**, **V-42** ([`apps/inventory/services/processing_workspace.py`](apps/inventory/services/processing_workspace.py); [`apps/inventory/processing_ops.py`](apps/inventory/processing_ops.py); [`frontend/src/pages/inventory/processing/`](frontend/src/pages/inventory/processing/)).
...
- **Inventory / Item Processor (pricing visibility)** — Read-only **Manifest pricing audit** accordion on **`ProcessingWorkspacePage`** loads **`GET /api/inventory/orders/{id}/manual-review/`** (paginated **`ManifestRow`** economics: **`unit_retail`**, allocated base, 2× ideal, set price, vs ideal). **`ManualReviewPanel`** adds **`readOnly`** mode and maps the **Retail** column from API **`unit_retail`** (not the unused **`retail_value`** label). **`ProcessingActiveCard`** shows **manifest line MSRP** and warns when a **pending** shelf price exceeds that line **`unitRetail`** ([`ProcessingWorkspacePage.tsx`](frontend/src/pages/inventory/processing/ProcessingWorkspacePage.tsx); [`ManualReviewPanel.tsx`](frontend/src/components/inventory/ManualReviewPanel.tsx); [`ProcessingActiveCard.tsx`](frontend/src/pages/inventory/processing/ProcessingActiveCard.tsx); [`inventory.types.ts`](frontend/src/types/inventory.types.ts) **`ManifestRow.unit_retail`**).
- **Inventory / Item Processor (workspace UI)** — **`ProcessingWorkspacePage`** rebuilt per processor mockup: **`ProcessingWorkspaceHeader`** (3-row breadcrumb, scanner search + session stats + slim progress), filter row without inline search (**Showing X of Y**, hide-dispositioned toggle default on, **Checked In** segment label, product-filter chip), sortable queue table with checkbox bulk selection and clearer empty states, **queue ↔ active card** swap in the main column, **`ProcessingBulkActionBar`** (merge vs bulk disposition by product coherence, batch dispute, swap when two rows selected), **`MergeModal`** / **`BulkDispositionModal`** / **`SwapModal`**, **`ProcessingWorkspaceFooter`** (**Close PO** + pending summary), **Print multiple** only on the active card, client **session log** lines merged with API **`session_log`**, active card **manifest / product / item** columns with salvage dispatch lock + checked-in banner + **Show all of this product**. Removed obsolete **`ProcessingBulkBar`** stub and legacy **`ProcessingTopBar`** (superseded by the header).
...
- **Steering / process** — **`review.0.Bump`** (**2026-05-02**): **`[Unreleased]`** Item Processor pricing audit + **`manual-review`** Retail column correction; Pass 1 (**directive**) vs Pass 2 plan remains as in **Changed** subsection above; **`context.md`** / **`consultant_context.md`** / **`extended/{frontend,inventory-pipeline}.md`** + initiative **Session 12**; extended TOC parity unchanged; **`frontend/package.json`** **`0.0.0`** unchanged; **no** `.version` bump (Part **2A** gate).
